export function physarumComputeShader(): string {
   return `
      @group(0) @binding(0) var<storage, read_write> waveArrayCurrent: array<f32>;
      @group(0) @binding(1) var<storage, read_write> waveArrayNew: array<f32>;
      ...
      fn cs(@builtin(global_invocation_id) global_id: vec3<u32>) {
         ...
         waveArrayNew[index] = center + 0.25 * ( left + right + up + down - 4 * center );
